...
flowchart TD
    ...
    %% Standard Path
    J --> K["Clean root settings.gradle (remove gradle.allprojects)"]
    K --> L[Prepend repositories template<br/>Use Gradle 6.x template when allprojects existed]
    L --> L1[Update gradle-wrapper.properties<br/>distributionUrl → Artifactory<br/>Version derived from existing Nexus URL]
    L1 --> S["Verify dependency resolution via init script<br/>resolveAllDeps with per-repo cache (-g)"]
    S -->|OK| S1["Insert env.GRADLE_PARAMS into Jenkinsfile(s)<br/>Paths from --jenkinsfiles"]
    S1 --> M[Commit changes<br/>Push to origin/branch-name]
    ...
    I6 --> I7[Verify dependency resolution via init script]
    I7 -->|OK| I8["Insert env.GRADLE_PARAMS into Jenkinsfile(s)"]
    I8 --> M
    I7 -->|Fail| N
    ...
    R3 --> R4[Verify dependency resolution via init script]
    R4 -->|OK| R5["Insert env.GRADLE_PARAMS into Jenkinsfile(s)"]
    R5 --> M
    R4 -->|Fail| N
    M --> O[Next repo]
    N --> O
